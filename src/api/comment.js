...
//分页查询评论
const query = (type, type_key, option) => {
...
//发表评论
const send = (type, type_key, user_id, content) => {
    return new Promise(async(resolve, reject) => {
        try {
            let model = new Model;
            const insert = {
...
//发表评论间隔和发帖总数限制
const sendLimit = (user_id) => {
    return new Promise(async(resolve, reject) => {
        try {
            let model = new Model;

            if(!commentConfig.enable) return resolve(true);
            ...
            if(results['lo_count'] >= limit){
                return resolve(commentConfig.limitInterval_msg);
            }
            ...
            if(results['lo_count'] > 0){
                return resolve(commentConfig.interval_msg);
            }
            resolve(true);
        }catch (err){
            reject(err);
...
//回复评论
const reply = (comment_id, reply_id, user_id, content) => {
    return new Promise(async(resolve, reject) => {
        try {
